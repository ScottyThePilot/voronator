...
pub fn uniform20000(c: &mut Criterion) {
  ...
  c.bench_function("uniform 20k", |b| {
    b.iter(|| {
      Triangulation::new(black_box(&points)).expect("No triangulation exists for this input.")
    })
  });
}
...
pub fn gaussian20000(c: &mut Criterion) {
  ...
  let points: Vec<Point> = (0..20000)
    .map(|_| Point {
      x: rng.sample::<f64, StandardNormal>(StandardNormal) * 1000.0,
      y: rng.sample::<f64, StandardNormal>(StandardNormal) * 1000.0
    })
    .collect();
